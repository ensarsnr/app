...
import { CANCEL, LABEL_SEARCH, QUANTITY } from "../constants/constText";
        ...
        <div className="card mb-2">
          ...
          >
            ...
            <div className="col-3 text-center">
              {e.quantity === "undefined" ? 1 : e.quantity}{" "}
              {QUANTITY.toLowerCase()} {e.product_name}
            </div>
            <div className="col-3" style={{ textAlign: "end" }}>
              {new Date(e.order_date).toLocaleString()}
            </div>
            <div className="col-3 text-end">
              <Button
                variant="danger w-75 p-2"
                onClick={() => deleteData(e.id)}
              >
                {CANCEL}
              </Button>
            </div>
            <div className="text-end mt-2 text-danger col-5">
              {e.select_coffee}
            </div>
          </div>
        </div>
